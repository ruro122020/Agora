...
The nested match is confusing because a single value with **two layers of
wrapping** comes out of `next()`, and each `match` opens one layer.
...
- The **outer** `match iter.next()` opens the `Option` layer: item or finished?
  Matching `Some(connection_result)` peels off the `Some` and binds what was inside to
  `connection_result`. But what was inside is `Ok(TcpStream)`, still wrapped. So that
  `connection_result` is a `Result`, not a connection yet.
- The **inner** `match connection_result` opens the `Result` layer: good connection or
  failure? `Ok(stream)` peels the `Ok` and binds the naked `TcpStream`. *Now*
  it's a connection.
